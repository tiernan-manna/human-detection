(function () {
  ...
  // URLs we've kicked off a fetch for but haven't received yet, so we
  // don't fire 30 concurrent fetches for the same JPEG when N tiles
  // happen to land on the same frame in the same tick.
  const _replayPrimingUrls = new Set();
  ...
  function sendReplayTile(tile) {
    if (!STATE.ws || STATE.ws.readyState !== WebSocket.OPEN) return;
    const frame = tile.manifest.frames[tile.playlist[tile.cursor]];
    if (!frame) return;

    // Always keep the visible thumbnail in sync, regardless of whether
    // the WS payload made it out this tick. This is what makes the
    // tiles look like videos playing — without it the user would only
    // see motion once preload caught up to a tile's cursor, which on a
    // big recording could be seconds.
    if (!tile.img.src.endsWith(frame.jpegUrl)) {
      tile.img.src = frame.jpegUrl;
      if (
        frame.imgW &&
        frame.imgH &&
        (tile.canvas.width !== frame.imgW || tile.canvas.height !== frame.imgH)
      ) {
    ...
    const cached = STATE.replayCache.get(frame.jpegUrl);
    if (!cached) {
      // First time we've reached this frame — fire a background fetch
      // so the next loop around the playlist is a hit. The visible
      // image already updated above, so playback is uninterrupted; we
      // just under-send by one tick on the very first cycle.
      primeReplayFrame(frame.jpegUrl);
  ...
  // sendReplayTile needs the JPEG bytes synchronously to send via WS, so
  // we maintain an in-memory cache keyed by URL. The browser HTTP cache
  ...
  function primeReplayFrame(url) {
    if (STATE.replayCache.has(url) || _replayPrimingUrls.has(url)) return;
    _replayPrimingUrls.add(url);
    fetch(url)
      ...
      .catch(() => {
        /* single missing frame doesn't break replay */
      })
      .finally(() => _replayPrimingUrls.delete(url));
  }
  ...
  // Walk the manifest once to find the seq range of frames whose
  // ORIGINAL live-detector pass produced at least one detection. Used
  // to power the "fit to detections" button so the operator can clip
  // a long recording to the slice where a human was actually visible
  // in a single click.
  ...
  function refreshWindowUi() {
    ...
    if (hasDetections) {
      els.ctlWindowFit.title =
        `Fit to seq ${det.min}–${det.max} (${det.count}/${det.total} frames had a detection in the original live pass).`;
    } else {
      els.ctlWindowFit.title =
        "No detections in this recording's live_results.jsonl, so there's nothing to fit to.";
    }
